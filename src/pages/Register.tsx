...
const Register = ({ onClose, onSwitchToLogin }: RegisterProps = {}) => {
  ...
  useEffect(() => {
    const checkRedirectResult = async () => {
      try {
        ...
        if (user) {
          handleRegisterSuccess(user.displayName || user.email);
        }
      } catch (error) {
        console.error("Error al procesar redirección:", error);
        toast({
          title: "Error",
          description: "Hubo un problema al procesar el registro con Google.",
          variant: "destructive",
        });
      }
    };
  ...
  const handleRegisterSuccess = (identifier: string | null) => {
    toast({
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      let errorMsg = "Hubo un problema al registrar con Google.";
      if (error.code === 'auth/popup-closed-by-user') {
        errorMsg = "Cerraste la ventana de registro.";
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center">
                ...
                <label htmlFor="terms" className="ml-2 block text-sm text-gray-600">
                  Acepto los <a href="#" className="text-[#599ACF]">términos y condiciones</a>
                </label>
              </div>
